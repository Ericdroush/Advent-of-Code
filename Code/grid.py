...
class Grid(object):
    ...
    def neighbors(self, node, diagonal_movement=DiagonalMovement.never):
        ...
        if y == 0 and self.passable_up_down_border:
            ...
            #EDR added weight check
            if self.walkable(x, y - 1) and self.nodes[y][x].weight >= self.nodes[y - 1][x].weight - 1:
        ...
        if x == self.width - 1 and self.passable_left_right_border:
            ...
            #EDR added weight check
            if self.walkable(x + 1, y) and self.nodes[y][x].weight >= self.nodes[y][x + 1].weight - 1:
